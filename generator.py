...
class Generator(tf.keras.utils.Sequence):
    ...
    def __init__(self, DATASET_PATH,CSV,X_COL,Y_COL, BATCH_SIZE=32, shuffle_images=True, image_min_side=32):
        ...
        self.load_image_paths_labels(DATASET_PATH, CSV,X_COL,Y_COL)
        self.create_image_groups()
    
    def load_image_paths_labels(self, DATASET_PATH, CSV,X_COL,Y_COL):
        
        classes = ['Latin', 'Korean', 'Japanese', 'Arabic', 'Bangla', 'Chinese']
        ...
        #fileCsv= pd.read_csv(CSV_PATH)
        
        #fileNames= fileCsv[X_COL]
        
        for index, row in CSV.iterrows():
            image_path=DATASET_PATH  +str(row[X_COL])
            if os.path.exists(image_path):
                self.image_paths.append(image_path)
                self.image_labels.append(row[Y_COL])
